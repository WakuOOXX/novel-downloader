...
顶部选书源分组（想省事就选带「校验可用」字样的分组），输入书名回车。结果边搜边上屏，随时可以点「停止」。

「只看相关结果」默认开启：很多小站无视搜索词返回热门书充数，开启后只有书名/作者/分类/简介里含关键词的结果才会显示；想看全量就把它去掉。

### 书源校验

点「✔ 校验书源」会对原始全量书源表做一次连通性体检（HTTP 200 判有效），跑完后在同目录生成有效书源表 `bookSource.good.json`，之后搜索/下载只扫有效源。下次再点校验，永远是重扫原始全量表、重新生成有效表覆盖旧的。校验中途点「停止」则本次作废，沿用旧表。
...
| `sel_state.json` | 记忆文件：选中的书 ID + 原始表路径/校验记录。删掉即重置 |
| `downloads/` | 下载输出目录，可在界面里改 |
| `docs/` | 项目官网源码（GitHub Pages） |

书源分组、关键词、输出目录都会在程序内记住，不用每次重设。
