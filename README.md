...
顶部「书源文件」下拉可勾选多个书源 JSON 文件（`shuyuan/` 目录下），勾选变化立即生效。点「✔ 校验书源」会对全部勾选文件逐个做连通性体检（每文件 64 并发、HTTP 200 判有效），每个文件跑完后在同目录生成有效书源表 `<文件名>.good.json`，搜索/下载只扫有效源。下次再点校验，永远是重扫原始全量表、重新生成有效表覆盖旧的。校验中途点「停止」则当前文件作废、已完成文件的 good 表保留。
...
| `shuyuan/*.json` | 放入多个书源 JSON，通过下拉勾选后可合并搜索 |
